...
class MyAccountManager(BaseUserManager):
    ...
    def create_superuser(self, first_name, last_name, email, username, password):
        ...
        # these will be extra as compared to user model
        user.is_admin = True
        user.is_active = True
        user.is_staff = True
...
class Account(AbstractBaseUser):
    ...
    # To let the model know that there is a custom manager
    objects = MyAccountManager()

    def full_name(self):
        return f"{self.first_name} {self.last_name}"

    # Returns the email in templates
    def __str__(self):
        return self.email

    # Permission to be returned if the user is admin (required for custom user model)
...
class UserProfile(models.Model):
    ...
    profile_picture = models.ImageField(
        blank=True, null=True, upload_to='userprofile/')
